...
class _BoardState extends State<BoardView> {
  ...
  */
  ...
  late double optimalWidth = taller
      ? (MediaQuery.of(context).size.width.toInt() / (game.boardSize + 2))
          .floor()
          .toDouble()
      : (MediaQuery.of(context).size.height.toInt() / (game.boardSize + 2))
          .floor()
          .toDouble();

  late double cardWidth = optimalWidth > 100 ? 100 : optimalWidth;
  late TextStyle defaultTextStyle = cardWidth < 100
    ...
    setState(() {
      cols = [
        for (int colOffset = 0; colOffset < game.boardSize; colOffset++)
          [
            for (int i = 0; i < game.tileCount; i += game.boardSize)
              tiles[i + colOffset]
          ]
      ];
      rows = [
        for (int o = 0; o < game.tileCount; o += game.boardSize)
          [for (int i = 0; i < game.boardSize; i++) tiles[o + i]]
      ];
      rowTotals = [...rows.map((l) => l.fold(0, (p, t) => p + t.outputValue))];
      colTotals = [...cols.map((l) => l.fold(0, (p, t) => p + t.outputValue))];
      rowSolutions = [
        ...rows.map((l) => l.fold(0, (p, t) => p + t.solutionValue))
      ];
      colSolutions = [
        ...cols.map((l) => l.fold(0, (p, t) => p + t.solutionValue))
      ];
    });
  }
    ...
    setState(() {
      tiles = [
        for (int i = 0; i < game.tileCount; i++)
          Tile(Game.rng(), index: i, isBlank: blanks.contains(i))
      ];
    });
    _tallyBoard();
  }
  ...
  */
  ...
  Widget solutionTile(int solution, int tally) {
    ...
    if (solved) {
      textStyle =
          textStyle.apply(color: tally == solution ? Colors.green : Colors.red);
    }
    return SizedBox(
      width: cardWidth,
      height: cardWidth,
  ...
  Widget _playingField() {
    List<Widget> rows = [
      for (int i = 0; i < game.boardSize; i++) _row(i * game.boardSize)
    ];
    return Column(
      children: rows,
    );
    ...
    return Column(
      children: [
        for (int i = 0; i < rows.length; i++)
          solutionTile(rowSolutions[i], rowTotals[i])
      ],
    );
  }
    ...
    return Row(
      ...
      children: [
        for (int i = 0; i < cols.length; i++)
          solutionTile(colSolutions[i], colTotals[i])
      ],
    );
  }
  ...
  Widget _userSideSums() {
    ...
    for (int i = 0; i < rows.length; i++) {
      kids.add(
          showHints ? answerTile(rowSolutions[i], rowTotals[i]) : blankTile());
    }
    return Column(
      children: kids,
    );
  ...
  Widget _userBottomSums() {
    ...
    for (int i = 0; i < cols.length; i++) {
      kids.add(
          showHints ? answerTile(colSolutions[i], colTotals[i]) : blankTile());
    }
    return Row(
      mainAxisAlignment: MainAxisAlignment.center,
      children: [
